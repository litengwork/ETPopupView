    ...
    private func sheetShowAnimation() -> ETPopupBlock {
        ...
        let block: (ETPopupView) -> Void = {
            ...
            if self?.superview == nil {
                ...
                self?.attachedView.mm_dimBackgroundView.addSubview(self!)
                self?.center.x = (self?.attachedView.center.x)!
                self?.center.y = (self?.attachedView.center.y)! + (self?.attachedView.frame.size.height)!
                self?.superview?.layoutIfNeeded()
            }
            ...
            UIView.animate(withDuration: (self?.animationDuration)!,
                           ...
                           options: [.curveEaseOut, .beginFromCurrentState],
                           animations: {
                            self?.center.y = (self?.attachedView.center.y)!/2 + (self?.attachedView.frame.size.height)!/2
                            self?.superview?.layoutIfNeeded()
    ...
    private func customShowAnimation() -> ETPopupBlock {
        ...
        let block: (ETPopupView) -> Void = {
            ...
            if self?.superview == nil {
                self?.attachedView.mm_dimBackgroundView.addSubview(self!)
                self?.center.x = (self?.attachedView.center.x)!
                self?.center.y = (self?.attachedView.center.y)! - 180
                self?.superview?.layoutIfNeeded()
            }
